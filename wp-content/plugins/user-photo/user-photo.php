...
function userphoto_the_author_thumbnail($before = '', $after = '', $attributes = array(), $default_src = '', $user_id = false){
    if (!$user_id) {
        global $authordata;
        if(!empty($authordata) && $authordata->ID)
            echo userphoto__get_userphoto($authordata->ID, USERPHOTO_THUMBNAIL_SIZE, $before, $after, $attributes, $default_src);
    } else {
        echo userphoto__get_userphoto($user_id, USERPHOTO_THUMBNAIL_SIZE, $before, $after, $attributes, $default_src);        
    }
}
...
function userphoto_resize_image($filename, $newFilename, $maxdimension, &$error){
	...
	//From WordPress image.php line 22
        ...
        /**
         removed by LP for transparency fix
