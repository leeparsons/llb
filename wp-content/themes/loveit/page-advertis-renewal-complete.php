<?php
	...
	if (isset($_SESSION['advert_payment'])) {
        ...
        if (!empty($query)) {
            $query2 = $wpdb->get_results($wpdb->prepare("SELECT renewal_cost, description, duration, quantity_duration FROM advert_options WHERE id = %s", $advertInfo['option_id']));
		...
		if ($success === true) {
			...
			if ((int)$query[0]->enddate < time()) {
				$tmpStartDate = strtotime('+1day');
			} else {
				$tmpStartDate = (int)$query[0]->enddate;
			}
            
			$startDate = $query[0]->startdate;
            
			//figure out the enddate
			$endDate = strtotime(date('d-M-Y', $tmpStartDate) . '+' .  $query2[0]->quantity_duration);
            if (isset($_SESSION['dev'])) {
echo $endDate;
die();
}
            ...
            $wpdb->update( 
						  ...
						  array( 
								...
								'enddatetime'	=>	date('Y-m-d', $endDate) . ' 09:00:00',
