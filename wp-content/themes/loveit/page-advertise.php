 ...
 */
	...
	if (isset($_POST['t'])) {
		...
		if (!isset($_POST['d']) || str_replace(' ', '', $_POST['d']) == '' || ((int)strtotime(str_replace(',', '', $_POST['d'])) == 0 || date('d, M, Y', strtotime(str_replace(',', '', $_POST['d']))) <= date('d, M, Y'))) {
    ...
    ?><script type="text/javascript" src="/wp-content/themes/loveit/js/jquery_ui.js"></script><link href="/wp-content/themes/loveit/css/date.css" rel="stylesheet"/><section class="col1"><?php
...
<form action="/advertise/" name="payment" class="aform fl w100" method="post" enctype="multipart/form-data">
	<p><strong>Please tell us some information about you</strong></p>
	<p>
...
<?php
	...
	<?php
		...
		?>
	<p>
		<label for="date">I would like my advert to start on (enter a date):</label>
...
<label for="choose-image-now">Choose an advertising image (200 pixels wide and 90 pixels high)</label><span class="clear"></span>
    ...
    <div class="uploadimage">
        ...
        <!--Having trouble with the image?<br/> Download a blank canvas template:-->
...
if (navigator.appVersion.indexOf("Mac") > -1 || navigator.appVersion.indexOf("X11") > -1 || navigator.appVersion.indexOf("Linux") > -1) {
        ...
        var uploader = new qq.FileUploader({
                                           ...
                                           $('#file-uploader').hide().after('<img src="' + responseJSON.fileName + '"/>');
                                            setTimeout(function() {$('.qq-upload-list').find('li:last').fadeOut();}, 1500);
