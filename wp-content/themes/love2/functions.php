<?php
    ...
    function love_excerpt($text = '', $readMore = true) {
    ...
    function love_image_cache($cachePrefix = 'large-cache-', $cacheWidth = 600, $imgClass = '', $asLink = false, $class = '', $textToDisplay = '', $file = '', $squareCrop = false, $justSrc = false, $echo = true) {
        ...
        $adjusted_width = floor($adjusted_width);
        $adjusted_height = floor($adjusted_height);
        $adjusted_width = floor($adjusted_width);
        ...
        if ($asLink === true) {
            ...
            $str .= '<a href="' . get_permalink() . '" class="' . $class . '">';
            
            if ($textToDisplay != '') {
                $str .= '<span>' . $textToDisplay . '</span>';
            }
        ...
        if ($useCacheImage === true) {
            if ($justSrc === true) {
                $str .= 'http://www.loveluxeblog.com' . str_replace($_SERVER['DOCUMENT_ROOT'], '', $cacheFile);
            } else {
                $str .= '<img width="' . $newWidth . '" height="' . $newHeight . '" src="' . str_replace($_SERVER['DOCUMENT_ROOT'], '', $cacheFile) . '" alt="' . get_the_title() . '" class="wp-post-image ' . $imgClass . '" />';
            }
        } else {
            $str = get_the_post_thumbnail('large');
        }
        unset($atts);
...
<?php endif;
    ...
    if ( ! function_exists( 'loveit_comment' ) ) :
    /**
     ...
     * @since Twenty Eleven 1.0
     */
    function loveit_comment( $comment, $args, $depth ) {
...
<?php echo esc_attr( get_the_author_meta( 'twitter', $user->ID ) ); ?>
        <table class="form-table">
            ...
            <tr>
                <th><label for="url2">Website</label></th>

                <td>
                    <input type="text" name="url" id="url2" value="<?php echo esc_attr(get_the_author_meta('url', $user->ID)) ?>" class="regular-text" /><br />
                    <span class="description">Please enter your website address</span>
                </td>

            </tr>

            <tr>
                <th><label for="blog">Blog URL</label></th>

                <td>
                    <input type="text" name="blog" id="blog" value="<?php echo esc_attr( get_the_author_meta( 'blog', $user->ID ) ); ?>" class="regular-text" /><br />
                    <span class="description">Please enter your Blog URL.</span>
                </td>
            </tr>
...
<?php }
    ...
    function my_save_extra_profile_fields( $user_id ) {
        ...
        update_usermeta( $user_id, 'blog', $_POST['blog'] );
