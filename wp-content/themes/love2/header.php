...
//    unset($MBD);
    ...
    if (is_single() && $current_user->ID == 0) {
        //update the views table!

        if (!$wpdb->query($wpdb->prepare("UPDATE wp_post_views SET last_viewed = NOW(), views = views + 1 WHERE day = CURDATE() AND postid =  %d", get_the_ID()))) {
            $wpdb->query($wpdb->prepare("INSERT INTO wp_post_views (last_viewed, day, views, postid) VALUES (NOW(), CURDATE(), 1, %d)", get_the_ID()));
        }
    }
...
<link rel="stylesheet" type="text/css" media="all" href="/wp-content/themes/love2/style.css" />
<?php
    if ($isMobile === true) {
...
<div id="page" class="hfeed">
    ...
    <?php
            ...
            ?><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home" class="site-title"><?php echo the_title() . ' - '; bloginfo( 'name' ); ?></a>
    <?php } ?>
        <nav id="access" role="navigation">
            <span class="assistive-text"><?php _e( 'Main menu' ); ?></span>
            ...
            <div class="skip-link"><a class="assistive-text" href="#secondary" title="<?php esc_attr_e( 'Skip to secondary content' ); ?>"><?php _e( 'Skip to secondary content' ); ?></a></div>
            ...
            <?php $cats = wp_nav_menu( array( 'theme_location' => 'primary', 'container' => '',   'menu_class'      => 'menu',  'echo' =>  false));
                ...
                echo substr($cats, 0, strlen($cats) - 11);
...
?><div class="section-wrap"><div class="content-left">