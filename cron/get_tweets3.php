<?php
    ...
    $sql = "SELECT id, tweettime FROM twitfeed WHERE feed = 'search_hash' AND lasttime <= NOW() - INTERVAL 4 MINUTE";
    ...
    if ($continue === true) {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, "http://search.twitter.com/search.json?q=loveluxelaunch&rpp=10&include_entities=true&result_type=mixed&show_user=true");
        ...
        foreach ($result->results as $stat) {
            ...
            unset($twitterarr);
            $sql .= ($sql == "") ? "" : ",";
            $sql .= "('" . mysql_real_escape_string($text) . "', NOW(), 'search_hash', '" . mysql_real_escape_string((string)$stat->created_at) . "', '" . mysql_real_escape_string(strtotime((string)$stat->created_at)) . "', '0', '" . mysql_real_escape_string((string)$stat->from_user) . "', '" . mysql_real_escape_string((string)$stat->profile_image_url) . "', '" . MD5(strip_tags((string)$stat->text)))  . "')";
        }
        ...
        if ($sql != "") {
            ...
            if ($continue === true) {
                ...
                if (is_array($tweets) && !empty($tweets)) {
                    mysql_query("DELETE FROM twitfeed WHERE feed = 'search_hash' AND id IN (" . implode($tweets, ',') . ")");
                }
